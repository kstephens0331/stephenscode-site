...
export default async function ServiceAreaPage({ params }: ServiceAreaPageProps) {
  ...
  return (
    <>
      ...
      {markdownContent ? (
        <section className="bg-slate-900 py-16 sm:py-24">
          <div className="mx-auto max-w-4xl px-6 lg:px-8">
            <div
              className="prose prose-lg max-w-none prose-invert prose-headings:text-white prose-p:text-gray-400 prose-li:text-gray-400 prose-strong:text-white"
            ...
            <div className="prose prose-lg max-w-none">
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                {area.neighborhoods.map((neighborhood, index) => (
                  <span
                    key={index}
                    className="inline-flex items-center rounded-full bg-slate-800 px-4 py-2 text-sm font-medium text-gray-300 shadow-sm shadow-black/20"
                  >
                    {neighborhood}
                  </span>
                ))}
            ...
            {serviceAreas
              ...
              .map((otherArea) => (
                <Link
                  ...
                  className="inline-flex items-center rounded-full bg-slate-800 px-4 py-2 text-sm font-medium text-gray-300 shadow-sm shadow-black/20 hover:bg-primary-500/10 hover:text-primary-700 transition-colors"
                >
                  {otherArea.name}
                </Link>
              ))}
      ...
      {/* Nationwide Remote Section */}
      <section className="bg-slate-900 py-12 sm:py-16">
        <div className="mx-auto max-w-4xl px-6 lg:px-8 text-center">
          <h3 className="text-2xl font-bold tracking-tight text-white">
            Not in {area.name}? We Work Nationwide.
          </h3>
          <p className="mt-4 text-base text-gray-400 max-w-2xl mx-auto">
            Through remote collaboration — video calls, screen sharing, and real-time communication — we deliver the same quality to clients anywhere in the United States. Same pricing, same guarantees, no matter where you are.
          </p>
          <div className="mt-8">
            <Link
              href="/contact"
